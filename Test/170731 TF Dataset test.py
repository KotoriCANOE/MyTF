...
tf.app.flags.DEFINE_integer('batch_size', 16,
                            """Batch size.""")
tf.app.flags.DEFINE_integer('buffer_size', 0, #8192,
                            """Buffer size for random shuffle.""")
tf.app.flags.DEFINE_boolean('pre_down', False, #True,
                            """Pre-downscale large image for (probably) higher quality data.""")
tf.app.flags.DEFINE_float('color_augmentation', 0, #0.05,
                            """Amount of random color augmentations.""")
tf.app.flags.DEFINE_float('noise_scale', 0.01, #0.01,
...
def inputs(files, is_training=False, is_testing=False):
    ...
    dataset = dataset.map(parse3_func, num_threads=threads,
                          output_buffer_size=threads * 64)
    if is_training and FLAGS.buffer_size > 0: dataset = dataset.shuffle(buffer_size)
    ...
    with tf.train.MonitoredTrainingSession(
        ...
        while not sess.should_stop():
            if step % FLAGS.log_frequency == 0:
                ...
                print('step {}: {} seconds, {} samples per second'.format(step, duration, sps))
                t = c_t
